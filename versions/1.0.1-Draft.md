...
2. The consumer chooses the levels (plans) they need, then both provider and consumer agree on the **SLA Agreement** document. See an example at [pro-petstore-sla.yml](./samples/petstore/pro-petstore-sla.yml).
...
While `plans`, `quotas` and `rates` are optional, there are some considerations:

- SLA Plan (i.e. `context.type` is set to `plans`): can have plans but not together with `quotas` and/or `rates`. Can have `quotas`, `rates` or both as long as `plans` is not present.
- SLA Agreement (i.e. `context.type` is set to `agreement`): cannot have plans and must have `quotas`, `rates` or both.
...
| consumer     | `string`    | **Optional** Consumer information, data about the entity that consumes the service. This field is **required** if the context type is `agreement`. |
