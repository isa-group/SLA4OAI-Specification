...
SLA extension can be mapped in an external YAML or JSON file. The OpenAPI specification main document (inside the `info` section) can link to SLA definitions using the standard extension and `$ref` linking mechanism:
...
The `base` keyword is proposed as the common base plan. All limits in base plan apply to all plans. Consequently, a specific plan inherits definitions from `base` but it can override any element (*availability*, *pricing*, *quota* or *rate*) to make it more concrete or relaxed.
...
| Field Name     | Type          | Description  |
| :------------- | :------------:| :------------|
| cost           | `number`      | **Optional** Unitns of cost associated with this service. Defaults to `0` if unspecified. |
...
| period         | `string`   |  **Optional** The period specified for the given limit; it should be one of the following possible values: `secondly`, `minutely`, `hourly`, `daily`, `monthly` or `yearly`. |
...
3. Data elements and interchange formats – Information interchange – Representation of dates and times [ISO-8601](https://en.wikipedia.org/wiki/ISO_8601)
